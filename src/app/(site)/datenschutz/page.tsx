...
export const metadata: Metadata = {
  title: "Datenschutzerklärung | Gartenhilfe",
...
export default async function DatenschutzPage() {
  ...
  const adresse = [imp.strasse, `${imp.plz} ${imp.ort}`.trim()].filter(Boolean).join(", ");
      ...
      <div className="max-w-2xl mx-auto px-4 py-16">
        ...
        <DsSection title={`${nVerantwortlicher}. Verantwortlicher`}>
          ...
          <div className="mt-2 space-y-0.5">
            {imp.firmenname && <p className="font-medium">{imp.firmenname}</p>}
            {imp.inhaberName && <p>{imp.inhaberName}</p>}
            {adresse && <p>{adresse}</p>}
            {imp.telefon && <p>Tel.: {imp.telefon}</p>}
            {imp.email && (
              <p>E-Mail:{" "}
        ...
        {s.kontaktformular.enabled && (
          <DsSection title={`${nKontakt}. Kontaktformular`}>
            ...
            <ul className="mt-2 list-disc list-inside space-y-1">
              ...
              <li>Gewählte Leistung und Nachrichtentext</li>
            </ul>
            <p className="mt-3">
